...
export function CreateStaffDialog({
  ...
  useEffect(() => {
    if (open) {
      ...
      // Reset form and select current org by default
      setFormData({
        full_name: '',
        role: '',
        email: '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(data.error || 'Thêm nhân sự thất bại')
      }

      onOpenChange(false)
      onCreated?.()
    } catch (err: any) {
      setError(err.message || 'Thêm nhân sự thất bại')
    } finally {
      setIsLoading(false)
    }
  }
  ...
  const toggleOrg = (orgId: string) => {
    setFormData(prev => {
      ...
      if (isSelected) {
        ...
        return { ...prev, organization_ids: [...prev.organization_ids, orgId] }
      }
    })
  }
    ...
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50">
      <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full mx-4 overflow-hidden border border-slate-200 max-h-[90vh] flex flex-col">
        <div className="flex items-center justify-between p-6 border-b border-slate-100 bg-slate-50 shrink-0">
          <div>
            <h2 className="text-lg font-bold text-slate-900">Tạo tài khoản đăng nhập phần mềm</h2>
            <p className="text-xs text-slate-500 mt-0.5">Tạo tài khoản đăng nhập độc lập, không liên quan đến nhân sự đồng bộ APEC GLOBAL</p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 overflow-y-auto space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-sm font-semibold text-slate-800 mb-1.5">Email đăng nhập *</label>
              <input
                type="email"
            ...
            <div>
              <label className="block text-sm font-semibold text-slate-800 mb-1.5">Mật khẩu *</label>
              <input
                type="password"
